<template>
  <v-layout>
    <v-flex xs12>
      <v-card>
        <v-card-title>
          <h3 ml-0>Melbourne Health Principle Mobile</h3>
        </v-card-title>
        <v-card-text>
...
export default {
  ...
  methods: {
    async fetchData() {
      this.loading = true
      try {
        let response = await this.$store.dispatch('mhpmobile/fetch', { search: this.search, ...this.pagination })
    ...
    async deleteItem (item) {      
      try {
        ...
        let response = await this.$store.dispatch('mhpmobile/destroy', item.id)
        this.snackbar = true
    ...
    async importExcel() {
      ...
      try {
        let response = await this.$store.dispatch('mhpmobile/importExcel', excelFile)
    ...
    async exportExcel() {
      ...
      try {
        let response = await this.$store.dispatch('mhpmobile/exportExcel')
